...
def highenergycontinuation_exp(s,scrit,val_crit,deriv_crit,limit,t=0):
    """ goes like e^(-s/T(t)) """
...
def highenergycontinuation_poly(s,scrit,val_crit,deriv_crit,limit,n=1):
    """ goes like (1/s)^n """
    return limit + (limit-val_crit)/((scrit-((s**n)/(scrit**(n-1))))*(deriv_crit/(n*(limit-val_crit))) - 1)

def highenergycontinuation_log(s,scrit,val_crit,deriv_crit):
    """ goes/diverges like log(s) """
    return val_crit + deriv_crit*scrit*np.log(s/scrit)

def highenergycontinuation_trig(s,scrit,val_crit,deriv_crit,q):
