...
def crosssection_and_time(energy,theta,nucleus,**args):
        start_time=time.time()
        crosssection = crosssection_lepton_nucleus_scattering(energy,theta,nucleus,**args)
        end_time=time.time()
    ...
    if MPSentinel.Is_master():
        ...
        for method,N_partial_waves,atol,rtol,energy_norm,phase_difference_limit in itertools.product(*parameter_steps.values()): 
            ...
            if not skip:
                ...
                if pool_counter==N_pools:
                    print('Queued: ',parameter_counter+1,'/',N_params_iter)
                    pool.close()
                    pool.join()
                    pool_counter=0
                    ...
                    for key in results_dict:
                        if not (key in gotten_keys):
                            ...
                            crossections_difference=(crosssection_dict[key]-crosssection0)/crosssection0
